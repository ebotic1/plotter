...
td::String GraphDrawer::to_string(gui::CoordType x) {
    td::String s;
    s.format("%g", x);
    return s;
}
...
GraphDrawer::GraphDrawer(td::ColorID axisColor, gui::Font *axisFont, gui::Font *legendFont, bool startWithMargins):
...
void GraphDrawer::setUpDrawingWindow(){
...
void GraphDrawer::setAxisColor(td::ColorID boja){
    _axisColor = boja;
    _legenda.setTextColor(boja);
    drawAgain();
}

void GraphDrawer::setyAxisName(const td::String &yName)
{
    yAxisName = yName; 
...
void GraphDrawer::setxAxisName(const td::String &xName){
    xAxisName = xName; 
...
void GraphDrawer::setAxisNameDistance(double xNameDistanceFromAxis, double yNameDistanceFromAxis)
{
...
void GraphDrawer::getAxisNameDistance(double &xNameDistanceFromAxis, double &yNameDistanceFromAxis)
{
    xNameDistanceFromAxis = _xAxisNameSeperation;
    yNameDistanceFromAxis = _yAxisNameSeperation;
}

void GraphDrawer::setMargins(double left, double right, double bottom, double top)
{
...
void GraphDrawer::getMargins(double &left, double &right, double &bottom, double &top)
{
...
void GraphDrawer::showLegend(bool draw)
{
    _drawLegend = draw; 
    setUpDrawingWindow();
    drawAgain(); 
...
void GraphDrawer::addFunction(gui::CoordType *x, gui::CoordType *y, size_t length, td::ColorID color, double lineWidth, td::LinePattern pattern, td::String name)
{
    _funkcije.emplace_back(x, y, length, color, name, lineWidth, pattern);
    finishAddingFunction(_funkcije.back());
}

void GraphDrawer::addFunction(gui::CoordType* x, gui::CoordType* y, size_t length, double lineWidth, td::LinePattern pattern, td::String name){
    _funkcije.emplace_back(x, y, length, nextColor(), name, lineWidth, pattern);
    finishAddingFunction(_funkcije.back());
}


void GraphDrawer::addFunction(Function&& fun){
    _funkcije.emplace_back(std::move(fun));
    finishAddingFunction(_funkcije.back());
}

void GraphDrawer::finishAddingFunction(Function& newFun) {
...
void GraphDrawer::updateLimits(const Function& newFun){
...
void GraphDrawer::onSizeChanged()
{
    setUpDrawingWindow();
...
void GraphDrawer::setGraphSize(const gui::Size &sz)
{
    _size = sz;
    onSizeChanged();
}

void GraphDrawer::fitToWindow(){
    if (_Limits == nullptr)
        return;
...
void GraphDrawer::ZoomToWindow(const gui::Geometry& window){
    if (_funkcije.size() == 0)
        return;
...
void GraphDrawer::ZoomToArea(gui::CoordType* minX, gui::CoordType* maxX, gui::CoordType* minY, gui::CoordType* maxY){
    if (_funkcije.size() == 0)
        return;
    if (drawingWindow.size.width < 1 || drawingWindow.size.height < 1)
        return;
...
void GraphDrawer::changeWidth(double width, size_t function){
...
void GraphDrawer::changeName(const td::String& name, size_t function){
    if (checkRange(function))
        return;
...
void GraphDrawer::changePattern(td::LinePattern pattern, size_t function){
    if (checkRange(function))
        return;
...
void GraphDrawer::changeColor(td::ColorID color, size_t function){ // todo: modifokovati pastColors da reflektuje promjenu
    if (checkRange(function))
        return;
...
void GraphDrawer::setLegendLocation(const gui::Point& location)
{
...
void GraphDrawer::setLegendCols(int cols)
{
...
void GraphDrawer::removeColorFromAutopicker(td::ColorID color)
{
    _disallowedColors.emplace(color);
}

td::ColorID GraphDrawer::nextColor(){
...
void GraphDrawer::moveGraph(const gui::CoordType& x, const gui::CoordType& y)
{
    for (int i = 0; i < _funkcije.size(); ++i) {
...
void GraphDrawer::Zoom(const gui::CoordType &scale){
    if (scale < 0.0001)
        return;
